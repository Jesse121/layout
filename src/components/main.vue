  ...
  <div id="app">
    <mu-appbar>
      ...
      <mu-icon-button
        ...
        slot="right"
        href="https://github.com/jaweii/Vue-Layout"
      />
    </mu-appbar>
      ...
      <mu-col class="attributes" :width="width.attr" :tablet="width.attr" :desktop="width.attr">
        <mu-sub-header class="header">
          <mu-select-field class="select-field" autoWidth v-model="selectField.value">
            <mu-menu-item title="属性" value="属性"></mu-menu-item>
            <mu-menu-item title="组件树" value="组件树"></mu-menu-item>
          </mu-select-field>
          <span>
            <a
...
export default {
    ...
    parentComponent: {
      get() {
        ...
        if (this.current.parentId)
          component = this.$store.state.components.find(
            c => c.info.id === this.current.parentId
          );
        else component = null;
        return component;
      }
  ...
  methods: {
        ...
        case "Muse-UI":
          return window.open(
            "http://www.muse-ui.org/#/" +
              this.current.info.name.replace(" ", "")
          );
        default:
    ...
    createShare() {
      ...
      share.save().then(res => {
        this.share.url =
          location.origin + location.pathname + "#/share/" + res.id;
        this.share.experience =
          location.origin + location.pathname + "#/preview/pc/" + res.id;
      });
    }
  },
  components: {
